...
function [AcPower, Tz, batterystoredenergy, batchargseq, Bill] = SIPMPCLoad(Qinternal,T,IDIRW,IDIRE,IDIRN,IDIRS,IDIR,Tset,rnw,north,rsw,south,rew,east,rww,west,floor,roof,price)
...
ObjectiveFunction = @error_f_oc_bat;
 
nvars = 2*PredictionHorizon/TimeStep;    % Number of variables

LB = zeros(1,nvars);   % Lower bound    zeros for hvac
LB(nvars/2+1:nvars)=-1*ones(1,nvars/2);        % -1 for battery
UB = ones(1,nvars);  % Upper bound
intcon=1:nvars;
ConstraintFunction = @constraint_oc_bat;

options = gaoptimset;
% gaoptimset.Display= 'iter'
options.TimeLimit=50;

%options = gaoptimset(options,'TolCon',0.0000000001);
...
xx=[xx x'];
 ...
 batchargseq=[batchargseq x(nvars/2+1)]
 
 if x(1+nvars/2)==1;         % battry chargging
batterystoredenergy=[batterystoredenergy batterystoredenergy(n)+chargrate5*5*60]
stepcost=chargrate5*p(n)/500000;
totalcost=stepcost+totalcost;

elseif x(1+nvars/2)==0;     % battry off
batterystoredenergy=[batterystoredenergy batterystoredenergy(n)]
    
elseif x(1+nvars/2)==-1 && x(1)==1;    % battry dischargging and AC on
    if dischargrate5>performancecurve
       
        batterystoredenergy=[batterystoredenergy batterystoredenergy(n)-performancecurve*5*60];
         stepcost=-performancecurve*p(n)/500000;
        totalcost=stepcost+totalcost;
    else
        stepcost=-dischargrate5*p(n)/500000;
        totalcost=stepcost+totalcost;
       batterystoredenergy=[batterystoredenergy batterystoredenergy(n)-(dischargrate5)*5*60];
    end
    
elseif x(1+nvars/2)==-1 && x(1)==0;    % battry dischargging and AC off
        batterystoredenergy=[batterystoredenergy batterystoredenergy(n)-dischargrate5*5*60];
         totalcost=totalcost;
end   
    batterys0=batterystoredenergy;
