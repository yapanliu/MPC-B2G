...
global oc;
global T;
input=csvread('2.Predictionfile.csv'); % 288 rows, 288/24 = 12, 60/12 = 5; control time step 5 mins
input=input'; 
building=csvread('3.Buildinginformation.csv');
Qinternal=input(1,:); 
T=input(2,:);
IDIRW=input(3,:); % solar q ?
IDIRE=input(4,:); % east
IDIRN=input(5,:); % north
IDIRS=input(6,:); % south
IDIR=input(7,:); % roof
Tset=input(8,:)-1; % temperature setpoint 
price=input(10,:); 
oc=input(11,:); % occupancy state
...
[AcPowerBBS, TzoneBBS]= SIP_SS_BBS(Qinternal,T,IDIRW,IDIRE,IDIRN,IDIRS,IDIR,Tset,rnw,north,rsw,south,rew,east,rww,west,floor,roof);
figure;
