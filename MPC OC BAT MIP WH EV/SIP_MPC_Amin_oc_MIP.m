...
A_opt_T=zeros(nvars,nvars+1);
A_opt_T(1:nvars,1:nvars)=tril(ones(nvars,nvars));
b_opt_T=0;
c_opt_T=0;
slackx=zeros(2*nvars,1);
...
b_opt_T(m)=-m*ACOFF-Tz(n)+tsetpoint(1,n)+0.5+(1-oc(n))*(27-tsetpoint(1,n));
slackx(m)=-Tz(n)+tsetpoint(1,n);
b_opt_T(m+nvars)=m*ACOFF+Tz(n)-tsetpoint(1,n)+0.5+(1-oc(n))*(-23+tsetpoint(1,n));
slackx(m+nvars)=Tz(n)-tsetpoint(1,n);
end

A_opt_T=[A_opt_T;-A_opt_T];
A_opt_T(:,37)=slackx;
%TMIPobjfun=@(x) (x(1:nvars)*c_opt_T'+100000*x(nvars+1)+x(1));
c_opt_T=[c_opt_T 100];

%optimization
intcon=1:nvars;
LB = zeros(1,nvars+1);   % Lower bound
UB = ones(1,nvars+1);  % Upper bound

%[x,fval] = ga(TMIPobjfun,nvars+1,A_opt_T,b_opt_T,[],[],LB,UB,[],intcon,options);
%MIP T

ctype = 'UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU';
vartype = 'IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIC';
s = 1;

A_opt_T=A_opt_T;
param.msglev = 1;
param.itlim = 100;
[xmin,fmin,status,extra]=glpk (c_opt_T, A_opt_T, b_opt_T', LB, UB, ctype, vartype, s, param);
x=xmin;
xx=[xx x'];
