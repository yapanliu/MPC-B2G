% Created by Amin Zhaoxuan Li, Bing Dong, 12/21/2015
% this file can be used to design HVAC consumtion schedual for battery
% optimization
close all;
clear;clc;
...
[AcPowerMPC2, TzoneMPC2]= SIP_MPC_Amin2(Qinternal,T,IDIRW,IDIRE,IDIRN,IDIRS,IDIR,Tset,rnw,north,rsw,south,rew,east,rww,west,floor,roof,price);
figure;
...
BillMPCocAC= sum(AcPowerMPCocMIP(1:250).*price(1:250)/500000);
xxMPCocBMIP=xx;
xx_batMPCocBMIP=xx_bat;
figure;


%% Run Building Simulation  MPC with oc and b GA
% Use GA to solve MPC

[AcPowerMPCocBga, TzoneMPCocBga, batenergyMPCocBga, batchargseqMPCocBga, BillMPCocBga]= SIP_MPC_Amin_oc_bat(Qinternal,T,IDIRW,IDIRE,IDIRN,IDIRS,IDIR,Tset,rnw,north,rsw,south,rew,east,rww,west,floor,roof,price);

figure;
plot(AcPowerMPCocBga,'--*b','LineWidth',2);
title('AC','FontSize',16,'color','black');
xlabel('Time (Five Minutes)','FontSize',16,'color','black');
ylabel('W','FontSize',16,'color','black');
legend('MPC with occupancy and Battery GA');

figure;
hold on;
plot(TzoneMPCocBga,'--*b','LineWidth',2);
plot((Tset-32)*(5/9),'LineWidth',2);
plot(oc/2+23,'r');
...
xxMPCocBga=xx;
xx_batMPCocBga=xx_bat;

BillMPCocBga=BillMPCocBga;
BillMPCocACB= sum(AcPowerMPCocBga(1:250).*price(1:250)/500000)
