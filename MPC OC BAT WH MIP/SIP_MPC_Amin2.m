...
intcon=1:nvars;
ConstraintFunction = @constraint2;
...
[x,fval,exitflag,output,population] = ga(ObjectiveFunction,nvars,[],[],[],[],LB,UB,ConstraintFunction,intcon,options);
xx=[xx x'];
    
InitialPopulation=zeros(size(population));
for m=1:nvars-1
   InitialPopulation(:,m)=population(:,m+1);
end
